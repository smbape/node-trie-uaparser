...
const train = (data, options, tokensCb) => {
    ...
    data.forEach(agent => {
        ...
        const tokens = tokenizer.getTokens(ua, Object.assign({}, options, {
            lnames: lnames.filter(token => token !== "--"),
            ontoken() {
...
const addIndex = (agent, tokens, indexes, options) => {
    ...
    for (let i = tokens.length - 1, token; i >= 0; i--) {
        ...
        if (hasProp.call(tokenizer.tokenAliasMap, token)) {
            token = tokenizer.tokenAliasMap[token];
        }
...
Object.assign(exports, {
    train: opts => {
        ...
        const mandatoryTokenizerOptions = Parser.mergeVersionTokernizer(Object.assign({}, Tokenizer.defaultOptions), Parser.defaultOptions);

        ["knownFiniteVersionedTokens", "knownInfiniteVersionedTokens", "knownUnversionedTokens", "knownIrrelevantTokens"].forEach(prop => {
            if (!hasProp.call(mandatoryTokenizerOptions, prop)) {
                return;
            }

            const hashmap = flip(mandatoryTokenizerOptions[prop].map(token => token.toLowerCase()), 1, true);
            Object.keys(hashmap).forEach(token => {
                switch (prop) {
                    case "knownIrrelevantTokens":
                        mandatoryTokens[token] = TOKEN_TYPES.IRRELEVANT;
                        break;
                    case "knownFiniteVersionedTokens":
                        mandatoryTokens[token] = TOKEN_TYPES.FINITE_VERSION;
                        break;
                    case "knownInfiniteVersionedTokens":
                        mandatoryTokens[token] = TOKEN_TYPES.INFINITE_VERSION;
                        break;
                    case "knownUnversionedTokens":
                        mandatoryTokens[token] = TOKEN_TYPES.UNVERSIONED;
                        break;
                    default:
        ...
        let os = {
            ...
            file: opts.out.os,
            indexes: createIndex({
                tokens: Object.assign({}, mandatoryTokens)
            }),
        ...
        software.indexes.tokens = Object.assign({}, os.indexes.tokens, software.indexes.tokens);
        os.indexes.tokens = Object.assign({}, software.indexes.tokens, os.indexes.tokens);
        ...
        os.indexes.tokens = tokens;
        software.indexes.tokens = tokens;
        ...
        const stack = [
            mandatoryTokens,
            os.indexes.children,
            software.indexes.children,
            os.tokenizer.DLknownIrrelevantTokens,
            software.tokenizer.DLknownIrrelevantTokens,
            os.tokenizer.tokenAliasMap,
            software.tokenizer.tokenAliasMap,
        ];
