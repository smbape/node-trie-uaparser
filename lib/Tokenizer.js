...
class Tokenizer {
    constructor(options) {
        ...
        this.init(options);
        this.updateTokens(options.tokens, options);
    }
    ...
    init(options) {
        ...
        const trie = new Trie(this.LknownTokens);

        this.matcher = new RegExp([
            "(?:",
                `(?<=^|${ DELIMITER_CHARS_REG.source })`,
                `${ trie.toRegExp(true).source }`,
         ...
         if (this.LknownVersionedTokens.length !== 0) {
            const regexp = (new Trie(this.LknownVersionedTokens)).toRegExp().source;
    ...
    addToken(token, version, tokens, prev, ua, uaMatch, uaStart, uaEnd, options) {
        ...
        if (hasProp.call(this.DLknownVersionedTokens, token)) {
            tokens.push(version ? [token, version] : token);
            state = version ? 2 : 1;
        } else if (hasProp.call(this.DLknownTokens, token)) {
            tokens.push(token);
            state = 1;
        } else if (options.lnames && options.lnames.some(lname => lcs(token, lname).length >= (lname.length >= 5 ? 5 : lname.length))) {
            tokens.push(token);
            state = 1;
            type = 2;
            ...
            if (/^[\s/(]+$/.test(prev) && /^[-+_]?\d.*$/.test(token)) {
                if (Array.isArray(lastToken)) {
                    if (hasProp.call(this.DLknownVersionedTokens, lastToken[0])) {
                        tokens[tokens.length - 1] = [lastToken[0], `${ lastToken[1] }/${ token }`];
                    } else {
                        // A token without meaning
                        if (options.ontoken) {
                            options.ontoken(token, version, 0, uaMatch.index, ua, uaStart, uaEnd, tokens);
                        }
                        return 0;
                    }
    ...
    conflict(tokenizer, tokens1, tokens2, options) {
        const conflicts = [];
        const errors = [];
        ...
        tokenizer1.updateTokens(tokens1_, Object.assign({
            errors
        }, options));
        ...
        tokenizer2.updateTokens(tokens2_, Object.assign({
            errors
        }, options));
