...
class Tokenizer {
    ...
    updateTokens(tokens, removed, options) {
        const keys = tokens == null ? [] : Object.keys(tokens);

        if (keys.length === 0 && (!removed || removed.length === 0)) {
            return;
        }

        let len, i, token, type;
        ...
        for (i = 0; i < len; i++) {
            ...
            switch (type) {
                ...
                case TOKEN_TYPE_INFINITE_VERSION:
                    delete this.DLknownIrrelevantTokens[token];
                    delete this.DLknownFiniteVersionedTokens[token];
                    this.DLknownInfiniteVersionedTokens[token] = 3;
                    delete this.DLknownUnversionedTokens[token];
                    break;
    ...
    setTokensType(prop, tokens, options) {
        const updatedTokens = options && options.tokens || Object.create(null);
    ...
    compressTokens(tokens, options) {
        ...
        for (let i = tokensLen - 1; i >= 0; i--) {
            ...
            versioned[token] = rversion ? rversion : version;
            tokens_[len++] = token;
        }
    ...
    addToken(token, version, tokens, prev, ua, uaMatch, options) {
        const ltoken = trimIrrelevant(token);
            ...
            if (/^[\s/(]+$/.test(prev) && /^[-+_]?\d.*$/.test(ltoken)) {
                if (Array.isArray(lastToken)) {
                    if (hasProp.call(this.DLknownVersionedTokens, lastToken[0])) {
                        ...
                        if (options.ontoken) {
                            options.ontoken(ltoken, 0, uaMatch.index, ua, tokens);
                        }
                        return 0;
                    }
        ...
        if (options.ontoken) {
            options.ontoken(tokens[tokens.length - 1], type, uaMatch.index, ua, tokens);
        }
    ...
    getTokens(ua, options) {
        ...
        let lastIndex = 0;
        let match, index, prev, tmp, token, version, delimiter;
        ...
        while (this.matcher.lastIndex < uaLen && (match = this.matcher.exec(ua))) {
            ...
            if (token == null) {
                index = match.index;
                token = ua.slice(lastIndex, index);
            }
            ...
            if (tmp === "//" && (token === "http:" || token === "https:")) {
                ...
                if (tmp) {
                    token = ua.slice(lastIndex, tmp.index);
                    this.matcher.lastIndex = NON_SLASH_DELIMITER_REG.lastIndex;
                } else {
                    this.matcher.lastIndex = uaLen;
                    token = ua.slice(lastIndex);
                }
            }

            if (this.addToken(token, trimNotVersion(version), tokens, prev, ua, match, options) === 1) {
                prev = delimiter;
            } else {
                prev = "";
            }
    ...
    conflict(tokenizer, tokens1, tokens2, removed1, removed2) {
        const conflicts = [];
        const errors = [];
        ...
        const tokens2_ = Object.create(null);
        Object.keys(tokens2).forEach(token => {
            token = token.toLowerCase();
            tokens2_[resolveToken(token)] = tokens2[token];
        });
        tokenizer2.updateTokens(tokens2_, removed2, {
            errors
        });
...
const mergeOptions = (tokenizer1, tokenizer2, options) => {
    ["knownFiniteVersionedTokens", "knownInfiniteVersionedTokens", "knownUnversionedTokens"].forEach(prop => {
        if (tokenizer2[prop]) {
            tokenizer1[prop] = options.reset || tokenizer1[prop] == null ? tokenizer2[prop] : tokenizer1[prop].concat(tokenizer2[prop]);
        }
    });

    ["knownIrrelevantTokens"].forEach(prop => {
        if (tokenizer2[prop]) {
            if (options.reset || tokenizer1[prop] == null) {
    ...
    ["tokenAliasMap", "tokens"].forEach(prop => {
        if (tokenizer2[prop] != null) {
            tokenizer1[prop] = Object.assign(Object.create(null), tokenizer1[prop], tokenizer2[prop]);
        }
    });
